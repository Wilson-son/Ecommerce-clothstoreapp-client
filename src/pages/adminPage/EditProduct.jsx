...
import { useUpdateProductMutation, useUploadImageMutation } from "../../redux/api/productApiSlice";

// ── Schema mirrors model: color is { name, hex } single object ───────────────
const productSchema = z.object({
  ...
  images:        z.string().min(1, "Product image is required"),
});

export default function EditProduct({ product, onClose }) {
  const [imgPreview, setImgPreview] = useState(product?.images?.[0]?.url || null);
  ...
  // Re-populate when product prop changes
  useEffect(() => {
    if (product) {
      reset(buildDefaults(product));
      setImgPreview(product.images?.[0]?.url || null);
    }
  }, [product, reset]);
  ...
  const onSubmit = async (data) => {
    try {
      ...
      let public_id  = product?.images?.[0]?.public_id || "";
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="overflow-y-auto p-6 space-y-4 flex-1">
          ...
          <div
            ...
            className={`border-2 border-dashed rounded-xl p-4 flex flex-col items-center justify-center cursor-pointer transition-all ${
              ...
              : errors.images ? "border-rose-300 bg-rose-50/20"
          ...
          {errors.images && <p className="text-xs text-rose-600 flex items-center gap-1"><FiAlertCircle />{errors.images.message}</p>}
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            {/* Stock — maps to countInStock via controller */}
            <div className="col-span-2">
...
// ── Helper: build form defaults from a product doc ───────────────────────────
function buildDefaults(product) {
  return {
    ...
    stock:         product?.countInStock  || "",   // stored as countInStock in DB
